...
import { detectIntent, extractDestination, parseDatesFromQuery, parseFlights, normalizeCountry, type Intent } from "@/lib/smart";
...
export async function POST(req: NextRequest) {
  ...
  // Για hotels/cars/tours κρατάμε καθαρό destination
  const dest = intent === "hotels" || intent === "cars" || intent === "tours"
  ...
  // Προσωρινά: αν η Agoda δεν έχει fix, μπορείς να την κόψεις εδώ:
  // partners = partners.filter(p => p.id !== "agoda");

  // 4) Build suggestions
  const suggestions = partners.slice(0, 3).map((p) => {
    // Για flights κάνε ελαφρύ parsing IATA (αν ο χρήστης έγραψε ATH-LON κ.λπ.)
    let url: string;
    if (intent === "flights") {
      const f = parseFlights(rawQuery);
      // Θα περάσουμε q σαν "ATH-LON" (το buildUrl μπορεί να αγνοήσει — ΟΚ για MVP)
    ...
    return {
      title: p.name,
      url,
      reason: reasonFor(p.id, intent, country)
    };
  });
